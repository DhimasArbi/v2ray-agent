...
showInstallStatus() {
	if [[ -n "${coreInstallType}" ]]; then
		...
		if [[ -n ${currentInstallProtocolType} ]]; then
			echoContent skyBlue "Protocol installed: \c"
		fi
...
selectAcmeInstallSSL() {
    ...
    if [[ -n "${customPort}" ]]; then
        ...
        if [[ -z "${dnsSSLStatus}" ]]; then
            read -r -p "Whether to use DNS to apply for certificate [y/n]:" installSSLDNStatus
...
acmeInstallSSL() {
    if [[ "${dnsSSLStatus}" == "true" ]]; then
        ...
        if [[ -n "${txtValue}" ]]; then
			...
			echoContent yellow " ---> Please refer to this tutorial for adding methods, https://github.com/DhimasArbi/v2ray-agent/blob/master/documents/dns_txt.md"
...
customPortFunction() {
	...
	if [[ -z "${currentPort}" && -z "${customPort}" ]] || [[ "${historyCustomPortStatus}" == "n" ]];then
		...
		if [[ -n "${btDomain}" ]]; then
            echoContent yellow "Please enter the port [not the same as the BT Panel port]"
        else
            echoContent yellow "Please enter the port [default: 443], such as a custom port, only allow using DNS to request certificates [Enter use default]"
        fi
...
randomPathFunction() {
	...
	if [[ "${historyPathStatus}" == "y" ]]; then
		...
		echoContent yellow "Please enter a custom path [example: alone], no slashes required, [enter for random path]:"
		...
		if [[ -z "${customPath}" ]]; then
			...
			if [[ "${customPath: -2}" == "ws" ]]; then
                echo
                echoContent red " ---> Custom path endings cannot be used at the WS ending, otherwise the tap path cannot be distinguished"
...
nginxBlog() {
	...
	if [[ -d "${nginxStaticPath}" && -f "${nginxStaticPath}/check" ]]; then
		...
		if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
			...
			wget -q -P /usr/share/nginx https://raw.githubusercontent.com/DhimasArbi/v2ray-agent/master/fodder/blog/unable/html${randomNum}.zip >/dev/null
			unzip -o ${nginxStaticPath}${randomNum}.zip -d ${nginxStaticPath} >/dev/null
			rm -f ${nginxStaticPath}${randomNum}.zip*
...
installCronTLS() {
    if [[ -z "${btDomain}" ]]; then
        echoContent skyBlue "\nProgress $1/${totalProgress} : Add a scheduled maintenance certificate"
...
xrayVersionManageMenu() {
	...
	if [[ "${selectXrayType}" == "1" ]]; then
		...
		echoContent yellow "3.If the rolled back version does not support the current config, it will not be able to connect, operate with caution"
		echoContent skyBlue "------------------------Version-------------------------------"
		curl -s https://api.github.com/repos/DhimasArbi/Xray-core/releases | jq -r '.[]|select (.prerelease==false)|.tag_name' | head -5 | awk '{print ""NR""":"$0}'
		echoContent skyBlue "--------------------------------------------------------------"
		read -r -p "Please enter the version to be rolled back:" selectXrayVersionType
		version=$(curl -s https://api.github.com/repos/DhimasArbi/Xray-core/releases | jq -r '.[]|select (.prerelease==false)|.tag_name' | head -5 | awk '{print ""NR""":"$0}' | grep "${selectXrayVersionType}:" | awk -F "[:]" '{print $2}')
		if [[ -n "${version}" ]]; then
			updateXray "${version}"
		else
			echoContent red "\n ---> Input error, please re-enter"
...
updateGeoSite() {
    ...
    reloadCore
    echoContent green " ---> The update is complete"
...
updateXray() {
	...
	if [[ -z "${coreInstallType}" ]]; then
		if [[ -n "$1" ]]; then
			...
			version=$(curl -s https://api.github.com/repos/DhimasArbi/Xray-core/releases | jq -r ".[]|select (.prerelease==${prereleaseStatus})|.tag_name" | head -1)
		fi
...
Documentation=https://github.com/DhimasArbi/Xray-core
After=network.target nss-lookup.target
[Service]
User=root
ExecStart=${execStart}
...
customCDNIP() {
	...
	echoContent yellow "# Notes"
	echoContent yellow "\nTutorial address:"
	echoContent skyBlue "https://github.com/DhimasArbi/v2ray-agent/blob/master/documents/optimize_V2Ray.md"
...
defaultBase64Code() {
	...
	if [[ "${type}" == "vlesstcp" ]]; then
		...
		if [[ "${coreInstallType}" == "1" ]] && echo "${currentInstallProtocolType}" | grep -q 0; then
			...
			echoContent green "Protocol type: VLESS, address: ${currentHost}, port: ${currentDefaultPort}, user ID: ${id}, security: xtls, transmission method: tcp, flow: xtls-rprx-vision, account name: ${email}\n"
...
updateNginxBlog() {
	...
	echoContent red "=============================================================="
	echoContent yellow "# For customization, please manually copy the template file to ${nginxStaticPath} \n"
...
addCorePort() {
	...
	echoContent yellow "If hysteria is already installed, the new hysteria port will be installed at the same time"
	echoContent yellow "Entry example: 2053,2083,2087\n"

	echoContent yellow "1.View Added port"
...
updateV2RayCDN() {
	...
	if [[ -n "${currentAdd}" ]]; then
		...
		echoContent yellow "5. Remove CDN nodes"
		echoContent red "=============================================================="
...
updateV2RayAgent() {
	...
	if wget --help | grep -q show-progress; then
		wget -c -q --show-progress -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/DhimasArbi/v2ray-agent/master/install.sh"
	else
		wget -c -q -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/DhimasArbi/v2ray-agent/master/install.sh"
	fi
	...
	echoContent green " ---> Current version: v${version}\n"
	echoContent yellow "If the update is unsuccessful, please manually execute the following command\n"
	echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/DhimasArbi/v2ray-agent/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
	echo
	exit 0
}
	...
	if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "作者:mack-a"; then
		...
		if [[ -d "/usr/bin/" ]]; then
			if [[ ! -f "/usr/bin/menu" ]]; then
				ln -s /etc/v2ray-agent/install.sh /usr/bin/menu
				chmod 700 /usr/bin/menu
				vasmaType=true
			fi

			rm -rf "$HOME/install.sh"
		elif [[ -d "/usr/sbin" ]]; then
			if [[ ! -f "/usr/sbin/menu" ]]; then
				ln -s /etc/v2ray-agent/install.sh /usr/sbin/menu
				chmod 700 /usr/sbin/menu
				vasmaType=true
			fi
			rm -rf "$HOME/install.sh"
		fi
		if [[ "${vasmaType}" == "true" ]]; then
			echoContent green "Shortcut created successfully, executable [menu] to reopen the script"
		fi
	fi
}
...
ipv6Routing() {
    ...
    echoContent yellow "3. Uninstall IPv6 offload"
	echoContent red "=============================================================="
	...
	if [[ "${ipv6Status}" == "1" ]]; then
		...
		echoContent yellow "8.Input example: google,youtube,facebook,cn\n"
...
blacklist() {
    ...
    echoContent yellow "2. Add a domain name"
    echoContent yellow "3. Remove blacklist"
	echoContent red "=============================================================="
...
warpRouting() {
	...
	echoContent yellow "1. View diverted domains"
    echoContent yellow "2. Add a domain name"
    echoContent yellow "3. Uninstall WARP offload"
	echoContent red "=============================================================="
...
dokodemoDoorUnblockStreamingMedia() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow "# Notes"
	echoContent yellow "Any door unlock details, please check this article [https://github.com/DhimasArbi/v2ray-agent/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n"
...
customXrayInstall() {
	...
	if [[ -z ${selectCustomInstallType} ]]; then
		...
		installTools 1
		if [[ -n "${btDomain}" ]]; then
            echoContent skyBlue "\Progress  3/${totalProgress} : Pagoda panel is detected, skip the step of applying for TLS"
            handleXray stop
            customPortFunction
        else
			# apply for tls
...
xrayCoreInstall() {
	...
	installCronTLS 10
	if [[ -n "${btDomain}" ]]; then
        echoContent skyBlue "\Progress  11/${totalProgress} : Pagoda panels are detected, whether to install/reinstall the camouflage site？"
        echoContent red "=============================================================="
        echoContent yellow "# Notes"
        echoContent yellow "The static directory under the current installation website will be emptied, if it has been custom installed, please select it [n]\n"
...
hysteriaVersionManageMenu() {
    ...
    if [[ ! -d "/etc/v2ray-agent/hysteria/" ]]; then
        echoContent red " ---> The installation directory is not detected, please execute the script to install the contents"
        menu
        exit 0
    fi
    echoContent red "\n=============================================================="
